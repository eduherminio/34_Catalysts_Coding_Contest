            ...
            while (true)
            {
                ...
                foreach (var task in input.Tasks)
                {
                    var allowedMinutes = input.Price
                        ...
                        .Where((minute) => minuteConsumption[task.AllowedStartInterval + minute.index] < input.MaxPowerPerMinute)
                        .ToList();
                ...
                if (totalElectricityBill < Convert.ToUInt64(input.MaxElectricityBill) && input.Tasks.All(t => t.Completed))
                {
                    break;
                }
